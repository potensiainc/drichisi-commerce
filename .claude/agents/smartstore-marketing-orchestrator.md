...
### Threads (@drichisi_) - 2026-05-09 업데이트
- **목적**: 팔로워 유입 → 프로필 링크 → 스토어
- **콘텐츠**: 품격/부자/공간 인사이트 (상품 언급 절대 금지)
- **빈도**: 매일 1-2회
- **스타일**: 숫자 리스트형 (7-10개), 담백한 톤, 이모지 없음
- **역대 성과**: 1.7만 조회 (진짜 부자 옷장), 1.1만 조회 (돈 많아 보이는 집)

**S++ 보장 제목 형식:**
- `진짜 부자 [공간]에 꼭 있는 N가지` → 1.7만 조회
- `[여유/돈 많아 보이는] 사람 [공간]에 꼭 있는 N가지` → 1만 조회

**검증된 공간:** 옷장, 주방, 집(거실), 책상, 가방

**⭐ 창의적 확장 원칙 (2026-05-09 신설)**
- **70:30 법칙**: 검증된 패턴 70% + 신규 실험 30%
- **매주 2개 신규 주제 필수**: 완전 새로운 공간/주체/형식 시도
- **신규 공간 탐색**: 현관, 침실, 화장실, 베란다, 차 안, 지갑 속, 냉장고
- **신규 주체 탐색**: 감정(불안 없는), 관계(오래가는 친구), 시간(아침형), 계절(가을에 시작)
- **신규 형식 실험**: 비교형, 시간변화형, 계절형
- **실험 평가**: C등급까지 허용, D등급만 폐기, A등급 이상 시 변형 3개 추가
